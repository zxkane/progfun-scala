...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase().groupBy((element : Char) => element).mapValues(v => v.length()).toList.sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    sentenceAnagrams2(sentenceOccurrences(sentence))
  }

  def sentenceAnagrams2(o: Occurrences): List[Sentence] = {
    if (o.isEmpty) {
      List(Nil)
    } else {
      ...
      for (i <- combs if dictionaryByOccurrences.keySet(i);
           ...
           s <- sentenceAnagrams2(subtract(o, i))) yield {j :: s}
    }
  }
}
