...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = b.isStanding && b.isLegal && goal.x == b.b1.x && goal.y == b.b1.y
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moves) #:: more => {
      val appending = explored + block
      val extra = newNeighborsOnly(neighborsWithHistory(block, moves), appending)
      
      extra ++ from(more ++ extra, appending)
    }
  }
